...
data <- read.csv(file="MangroveContinuous.csv", stringsAsFactors=FALSE)
```

A function to clean data, make sure taxon names match between tree and data, etc.

```{r, eval=TRUE}
mangrove.vector<-data[,2]
names(mangrove.vector)<-data[,1]
name.check(MyTree, mangrove.vector)

CleanData <- function(MyTree, mangrove.vector) {
  geiger::treedata(MyTree, mangrove.vector)
}
```
...
```{r, eval=TRUE}
VisualizeData <- function(MyTree, mangrove.vector) {
...
```{r, eval=TRUE}
BM1<- geiger::fitContinuous(MyTree,mangrove.vector,model="BM")
print(paste("The rate of evolution is",BM1$opt$sigsq,"in units of, mm^2/MY"))

#"The rate of evolution is 7.01885491927883 in units of, mm^2/MY"
```
...
OU1<-fitContinuous(MyTree,mangrove.vector,model="OU") #potential error here "Non-ultrametric tree with OU Model, using VCV method" <-- is this bad?
par(mfcol=(c(1,2)))
plot(MyTree,show.tip.label=FALSE)
ou.tree<-rescale(MyTree,model="OU",OU1$opt$alpha)
plot(ou.tree)
```
...
one.discrete.char<- read.csv(file="MangroveDiscrete.csv")
discrete.mangrove.vector<-one.discrete.char[,2]
names(discrete.mangrove.vector)<-one.discrete.char[,1]
reconstruction.info<-ace(discrete.mangrove.vector,MyTree,type="discrete",method="ML",CI=TRUE)
...
labeled.tree<-MyTree$node.label<-best.states #not sure if this is correct
nodeBased.OUMV<-OUwie(MyTree,mangrove.vector,model="OUMV",simmap.tree=FALSE,diagn=FALSE) #receiving error on this and not sure why
